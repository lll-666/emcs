 ...
 */
public class VMatch extends AbstractValidationFilter {
    ...
    public void doFilter(Map<String, Object> rule, Map<String, Object> validationResult) {
        ...
        if(vName != null && CommonConstant.Validation.VMATCH.equals(vName)){
            //获取要校验的字段名
            Object vCName = rule.get("vCName");
            //获取校验值
            Object vCValue = rule.get(vCName);
            //获取校验规则
            Object vRValue = rule.get("vRValue");
            //开始校验
            if(!Pattern.compile(vRValue.toString()).matcher(vCValue.toString()).matches()){
                //添加校验结果
