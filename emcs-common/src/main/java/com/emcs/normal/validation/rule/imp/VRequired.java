 ...
 */
public class VRequired extends AbstractValidationFilter {
    ...
    public void doFilter(Map<String, Object> rule, Map<String, Object> validationResult) {
        ...
        if(vName != null && CommonConstant.Validation.VREQUIRED.equals(vName)){
            //获取要校验的字段名
            Object vCName = rule.get("vCName");
            //获取校验值
            Object vCValue = rule.get(vCName);
            //开始校验
            if(vCValue == null || "".equals(vCValue.toString().trim())){
                //添加校验结果
